...
### 8.2 수영장 상세 (모달 또는 전용 페이지)
- X모드 상태 변경
- 업로드 파일 보기/다운로드
- 수영장 관리자에게 메시지 발송
- 수영장 홈페이지 바로가기 (swimnote.kr/[slug])
- **수영장 작업 중 홈으로 빠지거나 전체 화면 이탈 금지. 모달/뒤로가기 규칙 기존 패턴 유지.**

### 8.3 X모드 종료 시 홈페이지 처리
- `xmode_status = OFF` 전환 시 홈페이지 도메인 자동 중지가 불가능한 경우
- 슈퍼어드민에 즉시 알림: "**[수영장명] X모드 종료 — 홈페이지 도메인 해제 필요**"
- 슈퍼어드민이 수동으로 slug 비활성화 처리

---

## 8-A. 슈퍼어드민 수영장별 이벤트 알림함

### 8-A.1 구조
- 수영장마다 **독립된 이벤트 알림함** 존재
- 대화창 형식: 이벤트가 위로 쌓임 (최신이 위)
- 첨부파일이 있는 경우 메시지 옆에 파일 아이콘 및 다운로드 버튼 표시
- 슈퍼어드민이 해당 이벤트에 직접 답장/메시지 발송 가능
...
| 가입 해지 | ✅ 즉시 푸시 | 구독 해지 |
| 문의사항 | ✅ 즉시 푸시 | 수영장 관리자 문의 (사진 첨부 가능) |
| 자료 제출 | ✅ 즉시 푸시 | 커리큘럼 의뢰 파일 등 자료 업로드 |
| 재결제/재등록 | ❌ 알림 없음 | 별도 리스트에서 확인 |
| 구독 변경 | ❌ 알림 없음 | 별도 리스트에서 확인 |

### 8-A.3 파일 업로드/다운로드 규칙
- 허용 확장자: 이미지(jpg, jpeg, png, gif, webp, heic), 문서(pdf, doc, docx, xls, xlsx, ppt, pptx, txt, csv), 영상(mp4, mov), 기타(zip)
- 최대한 유연하게 허용 (불필요한 차단 금지)
- 업로드 모달: 내용 작성 + 파일 첨부 동시 가능
- 슈퍼어드민/수영장 관리자 양방향 업로드 가능

### 8-A.4 수영장 관리자 앱 (설정 탭)
- 문의사항 작성 + 사진/파일 첨부
- 자료 제출 (커리큘럼 의뢰 파일 등)
- 슈퍼어드민 답장 확인
- 업로드 모달은 최대한 유연하게: 내용 자유 입력 + 다중 파일 첨부

### 8-A.5 검수 포인트
- [ ] 수영장 클릭 후 작업 중 홈/목록으로 빠지지 않는가
- [ ] 이벤트 알림함이 수영장마다 독립적으로 동작하는가
- [ ] 첨부파일 메시지에 다운로드 버튼이 표시되는가
- [ ] 신규가입/X모드가입/해지/문의/자료제출 시 슈퍼어드민 즉시 알림이 오는가
