...
4. GPT may select verified evidence IDs, but model-authored prose must not be passed through as support guidance. Construct user-visible guidance from the selected verified answers so citing a valid ID cannot smuggle in an unsupported claim.
5. No evidence, an unknown selection, or no non-repeated verified answer means an evidence-insufficient response rather than a fabricated answer.
