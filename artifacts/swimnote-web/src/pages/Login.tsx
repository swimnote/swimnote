...
export default function Login() {
  ...
  // ── 수영장 전용: 3개 필드 한 번에 처리
  const handlePoolLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!poolPin) { setError("웹 접속 비밀번호를 입력해주세요."); return; }
    setLoading(true);
    try {
      const result = await login(poolEmail, poolPassword);
      if ((result as LoginResult).totp_required) {
        setError("이 계정은 앱 OTP 인증이 설정되어 있어 이 페이지에서 로그인할 수 없습니다.");
        setLoading(false);
        return;
      }
      if ((result as WebPinRequired).web_pin_required) {
        const webSess = (result as WebPinRequired).web_session;
        const user = await completeWebPinLogin(webSess, poolPin);
              ...
              <form onSubmit={handlePoolLogin} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="password"
                    value={poolPassword}
                    onChange={(e) => { setPoolPassword(e.target.value); setError(""); }}
                    placeholder="비밀번호 입력"
                    required
                ...
                <div>
                  ...
                  <input
                    type="password"
                    value={poolPin}
                    onChange={(e) => { setPoolPin(e.target.value); setError(""); }}
