// /super-login — SWIMNOTE 내부 운영자 로그인
// 공개 홈페이지에 링크 없음. SiteHeader/Footer 사용 안 함.
// 기존 super_admin TOTP flow 재사용.
...
export default function SuperLogin() {
  ...
  const redirectByRole = (role: string, poolId?: string | null) => {
    if (role === "super_admin") { navigate("/super"); return; }
    if (role === "pool_admin" && poolId) { navigate("/admin"); return; }
    navigate("/");
  };
        ...
        {step === "credentials" && (
          <>
            <div className="mb-8 text-center">
              ...
              <h1 className="text-[18px] font-bold text-[#0a0a0a]" translate="no">SWIMNOTE</h1>
              <p className="text-[13px] text-[#888] mt-1">내부 운영자 로그인</p>
            </div>
              ...
              <form onSubmit={handleCredentials} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-[#e5e5e5] text-[14px] focus:outline-none transition-colors"
                    style={{ "--tw-ring-color": PURPLE } as any}
                  />
                </div>
                <div>
