...
const ROLE_CFG: Record<SuperAdminRole, { label: string; color: string; bg: string }> = {
  super_admin:     { label: '슈퍼관리자', color: '#7C3AED', bg: '#EDE9FE' },
  senior_admin:    { label: '시니어관리자', color: '#0891B2', bg: '#ECFEFF' },
  read_only_admin: { label: '읽기전용', color: '#6B7280', bg: '#F3F4F6' },
};
    ...
    return (
      <Pressable style={[s.card, !acc.isActive && s.cardInactive, locked && s.cardLocked]}
        ...
        <View style={s.cardMeta}>
          ...
          <MetaItem icon="shield" label={acc.twoFactorEnabled ? '2FA 활성' : '2FA 없음'} color={acc.twoFactorEnabled ? '#059669' : '#DC2626'} />
          {acc.loginFailCount > 0 && <MetaItem icon="alert-triangle" label={`실패 ${acc.loginFailCount}회`} color={acc.loginFailCount >= 3 ? '#DC2626' : '#D97706'} />}
        </View>
      </Pressable>
    );
  };
    ...
    <SafeAreaView style={s.safe} edges={[]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {[
          ...
          { label: '활성', val: stats.active, color: '#059669' },
          { label: '2FA 활성', val: stats.twoFA, color: '#0891B2' },
            ...
            <Pressable style={m.sheet} onPress={() => {}}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={m.section}>
                  ...
                  <InfoRow label="2차 인증" val={selected.twoFactorEnabled ? '활성' : '비활성'} valColor={selected.twoFactorEnabled ? '#059669' : DANGER} />
                  <InfoRow label="로그인 실패" val={`${selected.loginFailCount}회`} valColor={selected.loginFailCount >= 3 ? DANGER : '#374151'} />
                  <InfoRow label="잠금 상태" val={isLocked(selected) ? `잠금 (${new Date(selected.lockedUntil!).toLocaleString('ko-KR', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })} 해제)` : '정상'} valColor={isLocked(selected) ? DANGER : '#059669'} />
                  <InfoRow label="계정 상태" val={selected.isActive ? '활성' : '비활성'} valColor={selected.isActive ? '#059669' : '#9CA3AF'} />
                </View>
                ...
                <View style={m.section}>
                  ...
                  <View style={m.actions}>
                    {!selected.twoFactorEnabled && (
                      <ActionBtn label="2FA 강제 활성" icon="shield" color="#059669" bg="#D1FAE5"
                        loading={actionLoading === `2fa-${selected.id}`}
                        onPress={() => doForceTwoFactor(selected)} />
                    )}
                    <ActionBtn label="권한 변경" icon="user-check" color={P} bg="#EDE9FE"
                      onPress={() => setRoleModal(true)} />
                    {isLocked(selected)
                      ? <ActionBtn label="잠금 해제" icon="unlock" color="#059669" bg="#D1FAE5"
                          onPress={() => doUnlock(selected)} />
                      : <ActionBtn label="계정 잠금" icon="lock" color={DANGER} bg="#FEE2E2"
                          onPress={() => setLockModal(true)} />
                    }
                    <ActionBtn label={selected.isActive ? '계정 비활성화' : '계정 활성화'}
                      icon={selected.isActive ? 'user-x' : 'user-check'}
                      color={selected.isActive ? '#D97706' : '#059669'}
                      bg={selected.isActive ? '#FEF3C7' : '#D1FAE5'}
                      onPress={() => { doToggleActive(selected); setSelected(null); }} />
                    {selected.loginFailCount > 0 && (
                      <ActionBtn label="실패 횟수 초기화" icon="refresh-cw" color="#6B7280" bg="#F3F4F6"
            ...
            <Pressable style={[m.sheet, { paddingBottom: 30 }]} onPress={() => {}}>
              ...
              {(Object.keys(ROLE_CFG) as SuperAdminRole[]).map(r => (
                <Pressable key={r} style={[m.roleRow, selected.role === r && { backgroundColor: ROLE_CFG[r].bg }]}
                  ...
                  {selected.role === r && <Feather name="check" size={16} color={ROLE_CFG[r].color} />}
                </Pressable>
              ))}
            </Pressable>
          </Pressable>
...
function InfoRow({ label, val, valColor = "#374151" }: { label: string; val: string; valColor?: string }) {
  return (
...
const s = StyleSheet.create({
  ...
  kpiCard:      { width: 80, paddingVertical: 8, paddingHorizontal: 10, backgroundColor: "#F9FAFB",
  ...
  cardLocked:   { borderLeftWidth: 3, borderLeftColor: "#DC2626" },
  ...
  lockBadge:    { paddingHorizontal: 7, paddingVertical: 2, borderRadius: 6, backgroundColor: "#FEE2E2" },
  lockTxt:      { fontSize: 10, fontFamily: "Inter_700Bold", color: "#DC2626" },
  inactiveBadge:{ paddingHorizontal: 7, paddingVertical: 2, borderRadius: 6, backgroundColor: "#F3F4F6" },
...
const m = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 13, fontFamily: "Inter_700Bold", color: "#374151", borderBottomWidth: 1,
                  borderBottomColor: "#F3F4F6", paddingBottom: 6, marginBottom: 2 },
  ...
  sessKill:     { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, backgroundColor: "#FEE2E2" },
  sessKillTxt:  { fontSize: 12, fontFamily: "Inter_600SemiBold", color: "#DC2626" },
  ...
  devCurrent:   { color: "#059669", fontFamily: "Inter_400Regular" },
  ...
  hourBtnActive:{ backgroundColor: "#DC2626", borderColor: "#DC2626" },
  hourTxt:      { fontSize: 13, fontFamily: "Inter_500Medium", color: "#374151" },
  ...
  dangerBtn:    { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16,
                  paddingVertical: 10, borderRadius: 10, backgroundColor: "#DC2626" },
