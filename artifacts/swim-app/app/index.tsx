  ...
  async function handleKakaoLogin() {
    ...
    try {
      ...
      if (e.error_code === "kakao_no_account" && e.kakao_info) {
        // Phase 1 backport: 즉시 signup 이동 금지 → 선택 제공
        const kParams = {
          ...
          [
            {
              ...
              onPress: () => router.push({
                ...
                params: {
                  kakaoId:           kParams.kakaoId,
                  kakaoProfileImage: e.kakao_info.profile_image ?? "",
                  kakaoName:         kParams.kakaoName,
                },
              } as any),
            },
            {
              text: "새로 가입하기",
              onPress: () => router.push({ pathname: "/(auth)/signup", params: kParams } as any),
            },
      ...
      // Phase 1 backport: raw e.message 노출 금지
      const rawMsg = e.message || "";
      const safeMsg = rawMsg && rawMsg !== "카카오 로그인에 실패했습니다."
        ? "카카오 로그인에 실패했습니다. 잠시 후 다시 시도해주세요."
        : rawMsg || "카카오 로그인에 실패했습니다.";
      setError(safeMsg);
    ...
    <View style={[s.root, { backgroundColor: "#fff" }]}>
      ...
      >
        ...
        <View style={s.signupCol}>
          ...
          {/* 소셜 버튼 — Phase 1 backport: 아이콘 + 텍스트 full-width */}
...
const s = StyleSheet.create({
  ...
  /* Phase 1 backport: 소셜 버튼 full-width */
  socialBtnCol: { gap: 10 },
  socialFullBtn: {
