/**
 * (admin)/feedback-settings.tsx — 피드백 기본설정
 *
 * 수영장 관리자가 설정하는 피드백 기본 문구 페이지
...
export default function FeedbackSettingsScreen() {
  ...
  function handleAdd() {
    ...
    if (!t) { setAddError("문구를 입력해주세요."); return; }
  ...
  function handleEdit() {
    ...
    if (!t) { setEditError("문구를 입력해주세요."); return; }
  ...
  return (
    <SafeAreaView style={s.safe} edges={[]}>
      ...
      <Modal visible={addVisible} transparent animationType="slide" onRequestClose={() => setAddVisible(false)}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={s.modalKV}>
          <View style={[s.modalBox, { paddingBottom: insets.bottom + 16 }]}>
            <Text style={s.modalTitle}>기본 문구 추가</Text>
      ...
      <Modal visible={editItem !== null} transparent animationType="slide" onRequestClose={() => setEditItem(null)}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={s.modalKV}>
          <View style={[s.modalBox, { paddingBottom: insets.bottom + 16 }]}>
            <Text style={s.modalTitle}>문구 수정</Text>
            <TextInput
      ...
      <ConfirmModal
        ...
        message={
          activeTab === "custom"
            ? `커스텀 카테고리의 모든 문구가 삭제됩니다.\n계속하시겠습니까?`
            : `${labels[activeTab]} 카테고리를 기본 문구로 되돌립니다.\n현재 수정 내용은 사라집니다.`
        }
        confirmText="초기화"
        cancelText="취소"
        destructive
      ...
      <ConfirmModal
        ...
        title="전체 초기화"
        message="설정한 모든 피드백 기본 문구가 삭제되고 기본값으로 복구됩니다.\n이 작업은 되돌릴 수 없습니다."
        confirmText="전체 초기화"
        cancelText="취소"
        destructive
