...
const GROUP_COLORS: Record<string, { color: string; bg: string; border: string }> = {
  weekly_1: { color: "#1F8F86", bg: "#F0FBF9", border: "#B2E0DC" },
      ...
      {loading ? (
        ...
        >
          ...
          {data.groups.length === 0 ? (
            ...
            data.groups.map(g => {
              const palette = GROUP_COLORS[g.type_key] || { color: "#6F6B68", bg: "#F6F3F1", border: C.border };
              return (
...
function CalcCell({ label, value, color, bold }: { label: string; value: string; color: string; bold?: boolean }) {
  return (
    <View style={{ alignItems: "center", flex: 1 }}>
      <Text style={{ fontSize: 10, color: "#6F6B68", marginBottom: 2 }}>{label}</Text>
...
const s = StyleSheet.create({
  ...
  extraCount:     { fontSize: 16, fontWeight: "700", color: "#6F6B68" },
  ...
  infoCard:       { backgroundColor: "#F6F3F1", borderRadius: 12, padding: 14, gap: 5 },
