...
// ── DELETE /students/:id/force-delete — 회원 즉시 완전 삭제 (복원 불가) ──────
router.delete("/students/:id/force-delete", requireAuth, requireRole("super_admin", "pool_admin"),
  async (req: AuthRequest, res) => {
    try {
      ...
      const parentUserId = (student as any).parent_user_id;

      // 1. 개인 사진 삭제 (Object Storage + DB)
      const photos = await db.execute(sql`SELECT id, storage_key FROM student_photos WHERE student_id = ${studentId}`);
      if (photos.rows.length > 0) {
        try {
          const { Client } = await import("@replit/object-storage");
          const client = new Client();
          await Promise.allSettled((photos.rows as any[]).map(p => client.delete(p.storage_key).catch(() => {})));
        } catch { /* 무시 */ }
        await db.execute(sql`DELETE FROM student_photos WHERE student_id = ${studentId}`);
      }
      ...
      // 3. 수영 일지 삭제
      await db.execute(sql`DELETE FROM swim_diary WHERE student_id = ${studentId}`).catch(() => {});

      // 4. parent_students 연결 삭제
      await db.execute(sql`DELETE FROM parent_students WHERE student_id = ${studentId}`);

      // 5. 학부모 계정 처리: 다른 자녀 없으면 완전 삭제
      if (parentUserId) {
        const others = await db.execute(sql`
          SELECT id FROM students WHERE parent_user_id = ${parentUserId} AND id != ${studentId} LIMIT 1
        `);
        if (others.rows.length === 0) {
          await superAdminDb.execute(sql`
            DELETE FROM parent_pool_requests WHERE parent_account_id = ${parentUserId}
          `).catch(() => {});
          await db.execute(sql`DELETE FROM parent_accounts WHERE id = ${parentUserId}`).catch(() => {});
        }
      }
      ...
      console.log(`[forceDelete] student=${studentId} parentUser=${parentUserId || "none"}`);
      res.json({ success: true, message: `${(student as any).name} 회원이 완전히 삭제되었습니다.` });
    } catch (err) {
