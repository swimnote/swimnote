...
// GET /admin/pool-teachers — 같은 pool 선생님 목록 (선생님도 접근 가능)
router.get("/pool-teachers", requireAuth, requireRole("super_admin","pool_admin","teacher"),
  async (req: AuthRequest, res) => {
    try {
      const poolId = await getAdminPoolId(req);
      if (!poolId) { res.status(403).json({ error: "수영장 없음" }); return; }
      const rows = (await superAdminDb.execute(sql`
        SELECT id, name, email, is_activated
        FROM users
        WHERE swimming_pool_id = ${poolId}
          AND role = 'teacher'
      ...
      `)).rows;
      res.json(rows);
    } catch (err) { console.error("[pool-teachers]", err); res.status(500).json({ error: "서버 오류" }); }
  }
);
...
router.post("/makeups/:id/handover", requireAuth, requireRole("super_admin","pool_admin","teacher"),
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!receiver_teacher_id) return res.status(400).json({ error: "receiver_teacher_id 필수" });
      ...
      `)).rows as any[];
      if (!receiverRow) return res.status(404).json({ error: "선생님을 찾을 수 없습니다." });
      ...
      // 이벤트 로그
      logPoolEvent({
        pool_id: poolId, event_type: "makeup_handover_credit",
        entity_type: "makeup_session", entity_id: req.params.id, actor_id: actor.userId,
        payload: { receiver_teacher_id, receiver_name: receiverName, student_name: mk.student_name, month },
      }).catch(console.error);

      res.json({ success: true });
    } catch (err) { console.error("[makeups/handover]", err); res.status(500).json({ error: "서버 오류" }); }
  }
);
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const autoRenewCancelled = Boolean(p.x_auto_renew_cancelled);
...
// ── GET /action-center ─────────────────────────────────────────────
router.get("/action-center", requireAuth, requireRole("pool_admin"), async (req: AuthRequest, res) => {
  const poolId = await getPoolId(req.user!.userId);
  if (!poolId) return res.status(403).json({ error: "수영장 정보를 찾을 수 없습니다." });

  let renewal_due: any[] = [];
  let long_absence: any[] = [];
  let empty_seats: any[] = [];

  try {
    renewal_due = (await db.execute(sql`
      SELECT id, name, membership_end_at,
             EXTRACT(day FROM membership_end_at - CURRENT_DATE)::int AS days_left
      FROM students
      WHERE swimming_pool_id = ${poolId}
        AND membership_end_at IS NOT NULL
        AND membership_end_at BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '14 days'
        AND status NOT IN ('withdrawn', 'suspended')
      ORDER BY membership_end_at ASC
      LIMIT 50
    `)).rows as any[];
  } catch (_e) {
    renewal_due = [];
  ...
  try {
    long_absence = (await db.execute(sql`
      SELECT s.id, s.name,
             COUNT(CASE WHEN a.status = 'absent' THEN 1 END)::int AS consecutive_absences,
             MAX(CASE WHEN a.status = 'present' THEN a.class_date END) AS last_attended_date
      FROM students s
      LEFT JOIN attendance a ON a.student_id = s.id
        AND a.class_date >= CURRENT_DATE - INTERVAL '30 days'
      WHERE s.swimming_pool_id = ${poolId}
        AND s.status NOT IN ('withdrawn', 'suspended')
  ...
  try {
    empty_seats = (await db.execute(sql`
      SELECT cg.id AS class_group_id, cg.name AS class_name, cg.day_of_week, cg.start_time,
             cg.capacity, COUNT(sch.student_id)::int AS student_count,
             (cg.capacity - COUNT(sch.student_id))::int AS empty_seats
      FROM class_groups cg
      LEFT JOIN student_class_history sch
        ON sch.class_group_id = cg.id AND sch.left_at IS NULL
      WHERE cg.swimming_pool_id = ${poolId}
        AND cg.capacity IS NOT NULL AND cg.capacity > 0
      GROUP BY cg.id, cg.name, cg.day_of_week, cg.start_time, cg.capacity
      HAVING cg.capacity > COUNT(sch.student_id)
  ...
  res.json({ renewal_due, long_absence, empty_seats });
});

// ── GET /students/:studentId/basic-progress ────────────────────────
router.get("/students/:studentId/basic-progress", requireAuth, requireRole("pool_admin", "teacher"), async (req: AuthRequest, res) => {
  const poolId = await getPoolId(req.user!.userId);
  if (!poolId) return res.status(403).json({ error: "수영장 정보를 찾을 수 없습니다." });

  try {
    const [student] = (await db.execute(sql`
      SELECT id, name, level, class_group_id, membership_end_at, payment_status
      FROM students WHERE id = ${req.params.studentId} AND swimming_pool_id = ${poolId}
    `)).rows as any[];
    if (!student) return res.status(404).json({ error: "학생을 찾을 수 없습니다." });

    const [classGroup] = student.class_group_id
      ? (await db.execute(sql`SELECT name FROM class_groups WHERE id = ${student.class_group_id}`)).rows as any[]
      : [];
    const [diaryCount] = (await db.execute(sql`
      SELECT COUNT(*)::int AS cnt FROM diary_entries WHERE student_id = ${student.id}
    `)).rows as any[];
    const [lastDiary] = (await db.execute(sql`
      SELECT id, title, created_at FROM diary_entries
      WHERE student_id = ${student.id} ORDER BY created_at DESC LIMIT 1
    `)).rows as any[];
    const [attendance] = (await db.execute(sql`
      ...
      FROM attendance
      WHERE student_id = ${student.id} AND class_date >= CURRENT_DATE - INTERVAL '30 days'
    `)).rows as any[];
    const [lastAttended] = (await db.execute(sql`
      SELECT MAX(class_date) AS last_date FROM attendance
      WHERE student_id = ${student.id} AND status = 'present'
    `)).rows as any[];

    res.json({
      student,
      class_name: classGroup?.name ?? null,
      diary_count: diaryCount?.cnt ?? 0,
      last_diary: lastDiary ?? null,
      attendance: attendance ?? { total: 0, present: 0, absent: 0 },
      last_attended_date: lastAttended?.last_date ?? null,
    });
  } catch (e) {
    console.error("[admin/basic-progress]", e);
    res.status(500).json({ error: "서버 오류가 발생했습니다." });
  }
});

// ── PATCH /students/:studentId/payment-status ──────────────────────
router.patch("/students/:studentId/payment-status", requireAuth, requireRole("pool_admin"), async (req: AuthRequest, res) => {
  const payment_status = req.body?.payment_status;
  if (!["PAID", "DUE", "UNPAID"].includes(payment_status)) {
    return res.status(400).json({ error: "유효하지 않은 결제 상태입니다." });
  }
  ...
  try {
    await db.execute(sql`
      UPDATE students SET payment_status = ${payment_status}
      WHERE id = ${req.params.studentId} AND swimming_pool_id = ${poolId}
    `);
    res.json({ success: true, payment_status });
  } catch (e) {
    console.error("[admin/payment-status]", e);
    res.status(500).json({ error: "서버 오류가 발생했습니다." });
  }
});
