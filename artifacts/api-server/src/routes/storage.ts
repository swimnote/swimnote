...
async function calcUserStorage(userId: string, poolId: string) {
  const [photo_bytes, photo_count, video_bytes, video_count, messenger_bytes,
    ...
    safeBytes(db.execute(sql`SELECT COALESCE(SUM(file_size),0)::bigint AS bytes FROM video_assets_meta WHERE uploaded_by=${userId} AND pool_id=${poolId} AND status='active'`)),
    safeCnt(db.execute(sql`SELECT COUNT(*)::int AS cnt FROM video_assets_meta WHERE uploaded_by=${userId} AND pool_id=${poolId} AND status='active'`)),
