  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (classGroupIds.length === 0) {
        return res.json({ success: true, summary: { total_revenue: 0, total_sessions: 0, total_makeup_sessions: 0, total_trial_sessions: 0, total_temp_transfer_sessions: 0 }, students: [], month });
      }

      // 이 달 출결 기록 조회 (정규 + 보강 + 임시이동 + 기타)
      ...
      for (const att of (attRows.rows as any[])) {
        ...
        if (att.session_type === "extra") {
          ...
          if (att.is_trial) {
            studentMap[att.student_id].trial_sessions++;
          } else {
            studentMap[att.student_id].regular_sessions++;
          }
        }
      }
      ...
      let totalRevenue = 0;
      let totalSessions = 0;
      let totalMakeupSessions = 0;
      let totalTrialSessions = 0;
      let totalTempTransferSessions = 0;
...
router.post("/settlement/save", requireAuth, requireRole("pool_admin", "teacher"),
  async (req: AuthRequest, res: Response) => {
    try {
      const { pool_id, month, summary, students, extra_manual_amount, extra_manual_memo } = req.body;
      ...
      const rows = await db.execute(sql`
        INSERT INTO monthly_settlements
          (id, pool_id, teacher_user_id, teacher_name, settlement_month, total_revenue, total_sessions,
           ...
           extra_manual_amount, extra_manual_memo, student_details, updated_at)
