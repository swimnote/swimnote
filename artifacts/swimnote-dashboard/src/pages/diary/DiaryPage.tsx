...
function DiaryDetailDrawer({
  ...
  useEffect(() => {
    if (detail && !editMode) setEditContent(detail.common_content ?? "");
  }, [detail, editMode]);
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div style={{ padding: "12px", background: "#F8FAFC", borderRadius: "6px", marginBottom: "16px", border: "1px solid #E2E8F0" }}>
                ...
                {detail.is_edited && (
                  <div style={{ marginTop: "8px", fontSize: "11px", color: "#94A3B8" }}>
                    수정됨 {detail.edited_at ? `(${formatDate(detail.edited_at)})` : ""}
                  </div>
                )}
                {detail.ai_generated && (
                  <div style={{ marginTop: "4px", fontSize: "11px", color: "#7C3AED", fontWeight: 600 }}>AI 자동 생성</div>
                )}
              </div>

              {/* Common content */}
              <div style={{ marginBottom: "16px" }}>
                <div style={{ fontSize: "11px", fontWeight: 600, color: "#94A3B8", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.5px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span>공통 수업 내용</span>
                  {!editMode && (
                    <button onClick={() => { setEditContent(detail.common_content ?? ""); setEditMode(true); setEditError(""); }}
                      style={{ background: "none", border: "1px solid #CBD5E1", borderRadius: "4px", padding: "2px 8px", fontSize: "11px", color: "#475569", cursor: "pointer" }}>
                      수정
                    </button>
                  )}
                </div>
                    ...
                    {detail.student_notes.map((note) => (
                      <div key={note.id} style={{ padding: "10px 12px", background: "#F8FAFC", borderRadius: "6px", border: "1px solid #E2E8F0" }}>
                        ...
                        <div style={{ fontSize: "13px", color: "#475569", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                          {note.note_text ?? note.content ?? "내용 없음"}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {detail.videos && detail.videos.length > 0 && (
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ fontSize: "11px", fontWeight: 600, color: "#94A3B8", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                    영상 ({detail.videos.length})
                  </div>
                  <div style={{ fontSize: "13px", color: "#64748B" }}>
                    영상 {detail.videos.length}개 첨부됨 — 앱에서 확인 가능합니다.
                  </div>
                </div>
              )}
...
export default function DiaryPage() {
  ...
  const { data, isLoading, isError } = useQuery<DiaryListResponse>({
    queryKey: ["diaries-admin", search, offset],
    queryFn: () => api.get(`/diaries/admin/all-entries?${params.toString()}`),
    keepPreviousData: true,
  } as Parameters<typeof useQuery>[0]);

  // Client-side date filter (server doesn't support date range on this endpoint)
  const entries = (data?.entries ?? []).filter((e) => {
    if (!dateFrom && !dateTo) return true;
    const d = e.lesson_date ?? e.created_at?.slice(0, 10);
    if (!d) return false;
    ...
    <div style={{ padding: "24px" }}>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <span style={{ fontSize: "12px", color: "#94A3B8", alignSelf: "center" }}>
          {isLoading ? "…" : `${entries.length}건 표시`}
        </span>
      </div>

      {/* Date filter note */}
      {(dateFrom || dateTo) && (
        <div style={{ padding: "8px 12px", background: "#FFFBEB", border: "1px solid #FCD34D", borderRadius: "6px", fontSize: "12px", color: "#92400E", marginBottom: "12px" }}>
          날짜 필터는 현재 페이지 내 결과에만 적용됩니다. 전체 기간 검색은 페이지를 넘기며 확인해주세요.
        </div>
      )}
      ...
      {isError ? (
        ...
        <div style={{ textAlign: "center", padding: "80px", color: "#94A3B8" }}>
          {search || dateFrom || dateTo ? "검색 결과가 없습니다." : "작성된 일지가 없습니다."}
        </div>
      ) : (
        <>
      ...
      {selectedId && (
        <DiaryDetailDrawer diaryId={selectedId} onClose={closeDrawer} />
      )}
    </div>
  );
}
